...
export default function BuyProductView({ idAcct, id, name, description, imageURL }){    
    ...
    const [orderHasError, setOrderHasErro] = useState(false)
    ...
    const addProductToCart = ()=> {
        cartApi.StoreInCart(idAcct, id)
        .then(
            () => {
                console.time('order')
                setIsAddedToCart(!isAddedToCart)
                console.log('Order stored in cart!')
                console.timeEnd('order')
            },
            () => {
                setIsAddedToCart(!isAddedToCart)
                setOrderHasErro(true)
            })
    }
                                            ...
                                            (
                                                ...
                                                (
                                                    <div>
                                                        ...
                                                        <div className="p-2 bg-indigo-800 items-center text-red-100 leading-none animate-pulse lg:rounded-full flex lg:inline-flex" role="warning">
                                                            <span className="flex rounded-full bg-red-500 uppercase px-2 py-1 text-xs font-bold mr-3">Error!</span>
                                                            <span className="font-semibold mr-2 text-left flex-auto">Product could not be added to cart. Try again later.</span>
